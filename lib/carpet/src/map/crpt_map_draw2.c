...
** the given positions.
*/
static void draw_block(const map_t *map, vec2_t screen_pos,
    vec2_t pos, window_t *win)
{
    color_t color = crpt_map_is_solid(map, pos) ? sfWhite : sfBlack;

    crpt_draw_square(win, screen_pos, map->tile_size, color);
}

/*
** Draws the player on the map.
*/
static void draw_player(const map_t *map, vec2_t screen_pos, window_t *win)
{
...
static void draw_outline(vec2_t *screen_pos, const map_t *map, window_t *win)
{
    crpt_draw_square(
        win, *screen_pos, 20.2 * map->tile_size,
...
void crpt_map_draw2(const map_t *map, vec2_t screen_pos, window_t *window)
{
    ...
    draw_outline(&screen_pos, map, window);
    ...
    for (double y = pos.y - off; y < pos.y + off; y += map->cube_size) {
        ...
        for (double x = pos.x - off; x < pos.x + off; x += map->cube_size) {
            draw_block(map, cur_screen, (vec2_t){ x, y }, window);
            cur_screen.x += map->tile_size;
        }
        cur_screen.y += map->tile_size;
    }
    draw_player(map, screen_pos, window);
}
